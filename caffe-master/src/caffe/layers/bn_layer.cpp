...
void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  shape.push_back(1);
    ...
    caffe_set(this->blobs_[3]->count(), moving_average_ ? Dtype(1) : Dtype(0), this->blobs_[3]->mutable_cpu_data());
  }
...
void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  // Mean normalization
  if (moving_average_ || this->phase_ == TEST) {
